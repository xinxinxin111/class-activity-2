...
student<-list(score=score,interest=interest) #list
#matrix
student_score<-rbind(score,interest)
colnames(student_score)<-c(1:100)
#data frame
df<-data.frame(score,interest)
```
...
br<-c(-1,59,70,80,90,100)
letters<-c("F","D",'C','B','A')
grade<-table(cut(score,br,letters))
grade_student<-cut(score,br,letters,order_result=TRUE)
df_interest<-data.frame(score,grade_student)
     
#cut score (ranking students score)
#cut(score,break=5,labels="E","D","C","B","A")
```

4. Now using the colorbrewer package (RColorBrewer) design a pallette and assign it to the groups in your data on the histogram.

```{r}
library(RColorBrewer)
#Let's look at the available palettes in RColorBrewer

#The top section of palettes are sequential, the middle section are qualitative, and the lower section are diverging.r
#Make RColorBrewer palette available to R and assign to your bins

#Use named palette in histogram
hist(score,c(-1,59,70,80,90,100),right=TRUE)
graph<-hist(score,c(-1,59,70,80,90,100),right=TRUE)
plot(graph,labels=c("F","D","C","B","A"),col=brewer.pal(5,"Blues"))
```
...
color<-brewer.pal(4,"Reds")
boxplot(score~grade_student,data=df,xlab="letter grade",ylab="score",main="score-interest",col=color)
...
```{r}
#Sepal.Length & Sepal.Width & Petal.Length & Petal.Width these four factors
plot(iris)
```
...
#file:///Users/xinxin/Desktop/hudk4050/class%20activity%202/class-activity-2.html
